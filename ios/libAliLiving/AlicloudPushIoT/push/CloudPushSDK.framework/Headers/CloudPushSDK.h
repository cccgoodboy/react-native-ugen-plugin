...
/* SDK版本号 */
#define MPUSH_IOS_SDK_VERSION @"1.9.5.3"
 ...
 *  @param     callback        回调
 */
+ (void)asyncInitWithSecurity:(CallbackHandler)callback;
...
/**
 *  配置标志位，指明App进入后台时，是否将推送消息通道关闭
 *  @param closed YES：主动断开通道（默认值） NO：不主动断开通道
 */
+ (void)setChannelStatusWhenEnterBackground:(BOOL)closed;
